    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/smarteditor/image_upload_html5.do'; 	//upload URL // REDBRUSH 업로드 URL 변경
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/smarteditor/image_upload.do',									//이미지 업로드 URL // REDBRUSH
 			/**
 			 * WEB/WAS 분리로 인해 resources 폴더 인식 불가로 경로 재설정
 			 */ 			
// 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.jsp',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소 // REDBRUSH
 	        sCallback : '/imageupload/se/quick_photo_uploader/popup/callback.jsp',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소 // REDBRUSH
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {
 	    		...
 	    		goReadyMode();
 	    		oFileUploader.reset();
 	    		window.close();			// REDBRUSH 업로드 성공후 팝업 닫기
 	    	},
